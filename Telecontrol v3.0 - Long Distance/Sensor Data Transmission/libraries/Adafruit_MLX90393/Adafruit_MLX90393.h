...
#define MLX90393_DEFAULT_ADDR (0x0C) /* Can also be 0x0C or 0x18, depending on IC */
...
#define MLX90393_BURST_SEL_SHIFT (6)
...
public:
  ...
  bool readRawMeasurement(uint8_t *data);
  bool readRawData(uint8_t *data);
  bool startSingleMeasurement(void);

  bool setBurstMode(uint8_t burst_sel, uint8_t burst_rate);
  bool startBurstMode();
  bool readBurstMeasurement(float *x, float *y, float *z);
  bool stopBurstMode();
  bool setOversampling2(enum mlx90393_oversampling oversampling);
