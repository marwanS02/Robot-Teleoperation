...
bool Adafruit_MLX90393::readRawData(uint8_t *data) {
  if (!startSingleMeasurement())
    return false;
  // See MLX90393 Getting Started Guide for fancy formula
  // tconv = f(OSR, DIG_FILT, OSR2, ZYXT)
  ...
  delay(mlx90393_tconv[_dig_filt][_osr] + 2);
  return readRawMeasurement(data);
}

bool Adafruit_MLX90393::readRawMeasurement(uint8_t *data) {
  uint8_t tx[1] = {MLX90393_REG_RM | MLX90393_AXIS_ALL};
  uint8_t rx[6] = {0};
...
bool Adafruit_MLX90393::setBurstMode(uint8_t burst_sel, uint8_t burst_rate) {
  uint16_t data;
  
  // Read current value of CONF2 register
  if (!readRegister(MLX90393_CONF2, &data)) {
      return false;
  }

  // Set BURST_SEL and BURST_DATA_RATE
  // data &= 0xFC00;  // Clear the lowest 10 bits
  // data |= (burst_sel & 0x0F) << MLX90393_BURST_SEL_SHIFT | ((burst_rate & 0x7F));
  data = 0x6380;
  // Serial.print("Data: ");
  // Serial.println(data);

  // Write back to CONF2 register
  return writeRegister(MLX90393_CONF2, data);
}

bool Adafruit_MLX90393::startBurstMode() {
  uint8_t tx[1] = {MLX90393_REG_SB | MLX90393_AXIS_ALL};  // Start burst with all axes

  uint8_t stat = transceive(tx, sizeof(tx), NULL, 0, 0);
  //return (transceive(tx, sizeof(tx), NULL, 0, 0) == MLX90393_STATUS_OK);

  // Serial.print("Status byte startBurstMode: ");
  // Serial.println(stat); //-> it prints an "error". I tested it two times: first time 0x18, second time 0x24

  if ((stat == MLX90393_STATUS_OK) || (stat == 32)) {
    return true;
  }
  return false;
}
...
bool Adafruit_MLX90393::readBurstMeasurement(float *x, float *y, float *z) {
  ...
  transceive(tx, sizeof(tx), rx, sizeof(rx), 0);

  int16_t xi = (rx[0] << 8) | rx[1];
  int16_t yi = (rx[2] << 8) | rx[3];
  int16_t zi = (rx[4] << 8) | rx[5];
...
bool Adafruit_MLX90393::stopBurstMode() {
  uint8_t tx[1] = {MLX90393_REG_EX};
  return (transceive(tx, sizeof(tx), NULL, 0, 0) == MLX90393_STATUS_OK);
}
 ...
 * @return True if the operation succeeded, otherwise false.
 */
 bool Adafruit_MLX90393::setOversampling2(
  enum mlx90393_oversampling oversampling) {
_osr = oversampling;
