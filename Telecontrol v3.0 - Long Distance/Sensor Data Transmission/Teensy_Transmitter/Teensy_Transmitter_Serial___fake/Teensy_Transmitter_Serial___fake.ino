...
void loop() {
  ...
  thetaw = readMagData();  // read MAG data (12 bytes) Huimin Modified should be 18 TBD
  bendVal_raw = readBendData();
  //thetaw = -30;  // read MAG data (12 bytes) Huimin Modified should be 18 TBD
  //bendVal_raw = 20;
...
float readMagData() {
  ...
  // // here we are doing the Geomagnetic Field compensation removing the contribution of Sensor 3
  ...
  // // Randomize if magnet is too far or inside the ring
  // // float norm_pos = 0.0;
  // // for (int i = 0; i <= 2; i++) {
  // //   norm_pos += x0[i] * x0[i];
  // // }
  // if( (std::sqrt(x0[0]* x0[0] + x0[1] * x0[1] + x0[2] * x0[2]) > 73.0/1000.0 ) || fabs(x0[0]) >= 0.033) {
  //   x0[0] = 0.0;
  //   x0[1] = -0.009;
  //   x0[2] = -0.0073;
  // }

  // thetaW = (fmod(atan2(x0[2]+DEV_RADIUS,x0[0]) , (2*PI)) - PI/2 )*180/PI;
  ...
  if (DEBUG) {
    ...
    Serial.print(thetaW);
    //Serial.print(", ");
    //Serial.print(end_time - starting_time);
    Serial.println();
    }
return thetaW;
}
