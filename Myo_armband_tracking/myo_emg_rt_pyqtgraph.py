...
# ---------- CONFIG ----------
PORT = "COM16"  # adjust as needed; e.g. "COM3" on Windows or "/dev/ttyUSB0" on Linux
BAUD = 115200
WINDOW_SEC = 5.0
FS_AGG = 200.0
N_STREAMS, N_CH = 4, 8
...
FS_IMU = 50.0  # Myo IMU ~50 Hz (used only as nominal frequency)
...
class EMGIMUWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # ---- Madgwick AHRS state ----
        self.ahrs = Madgwick(frequency=FS_IMU, gain=0.033)
        self.last_imu_ts_us = None      # last IMU timestamp in µs
        self.q_fused = None             # np.array([w,x,y,z]) from Madgwick
    ...
    def drain_queue(self, max_items=1000):
        ...
        while drained < max_items and not q.empty():
            ...
            if tag == 'e':
                ...
                gx_dps = gx / GYR_LSB_PER_DPS
                gy_dps = gy / GYR_LSB_PER_DPS
                gz_dps = gz / GYR_LSB_PER_DPS

                for i, v in enumerate((ax_ms2, ay_ms2, az_ms2)):
                    acc_buf[i].append(v)
                for i, v in enumerate((gx_dps, gy_dps, gz_dps)):
                    gyr_buf[i].append(v)

                # keep latest raw Myo quaternion (for comparison / fallback)
                quat_latest[0], quat_latest[1], quat_latest[2], quat_latest[3] = qx_f, qy_f, qz_f, qw_f

                # --- Madgwick fusion update ---
                # dt in seconds, robust to occasional jitter
                if self.last_imu_ts_us is None:
                    dt = 1.0 / FS_IMU
                else:
                    dt = (t_us - self.last_imu_ts_us) * 1e-6
                    dt = max(1e-4, min(0.1, dt))
                self.last_imu_ts_us = t_us

                # gyro in rad/s, accel in m/s^2
                d2r = math.pi / 180.0
                gyr_vec = Vector3D(gx_dps * d2r, gy_dps * d2r, gz_dps * d2r)
                acc_vec = Vector3D(ax_ms2, ay_ms2, az_ms2)

                # update AHRS and store fused quaternion as [w,x,y,z]
                self.ahrs.update(gyr=gyr_vec, acc=acc_vec, dt=dt)
                self.q_fused = self.ahrs.q  # keep Quaternion object, handle later
    ...
    def update_imu_plots(self):
        ...
        # orientation labels (prefer fused Madgwick, fallback to raw Myo)
        use_fused = self.q_fused is not None

        if use_fused:
            q = self.q_fused
            # Try common attribute conventions in Quaternion
            if hasattr(q, "w") and hasattr(q, "x"):
                w, x, y, z = float(q.w), float(q.x), float(q.y), float(q.z)
            elif hasattr(q, "q0") and hasattr(q, "q1"):
                w, x, y, z = float(q.q0), float(q.q1), float(q.q2), float(q.q3)
            elif isinstance(q, (tuple, list)) and len(q) == 4:
                w, x, y, z = map(float, q)
            else:
                # Fallback to raw Myo quaternion if we can't interpret it
                qx, qy, qz, qw = quat_latest
                w, x, y, z = qw, qx, qy, qz
                use_fused = False  # so label will say "Myo raw" below
            src = "Madgwick" if use_fused else "Myo raw"
        else:
            qx, qy, qz, qw = quat_latest
            w, x, y, z = qw, qx, qy, qz
            src = "Myo raw"
        ...
        self.lbl_q.setText(f"{src} q (w,x,y,z): {w:.3f}, {x:.3f}, {y:.3f}, {z:.3f}")
        self.lbl_rpy.setText(
            f"RPY (deg):  R={roll:+6.1f}  P={pitch:+6.1f}  Y={yaw:+6.1f}"
        )
