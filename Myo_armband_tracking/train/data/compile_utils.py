...
def build_dataset(sessions_root, out_dir, filename, fs=200, window_len=200, hop_len=30, envelope_len=50, 
    ...
    # --- Save provenance for each window so we can re-load raw later ---
    sess_arr = np.array([m[0] for m in meta_list], dtype="U")   # session folder name
    i0_arr   = np.array([m[1] for m in meta_list], dtype=np.int64)
    i1_arr   = np.array([m[2] for m in meta_list], dtype=np.int64)

    np.savez_compressed(out_npz, X=X, y=y, sess=sess_arr, i0=i0_arr, i1=i1_arr, meta=json.dumps(meta))
